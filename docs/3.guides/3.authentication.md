...
```go
import "github.com/zoobzio/rocco/auth0"
...
See the [Authentication Cookbook](../4.cookbook/2.authentication.md) for complete Auth0 examples including user enrichment patterns.

### GitHub Integration

For GitHub OAuth token authentication, use the `rocco/github` package:

```go
import "github.com/zoobzio/rocco/github"
...
The github package:
- Validates tokens via GitHub API (`/user`, `/user/orgs`)
- Caches user data (default: 5 minutes)
- Extracts scopes from `X-OAuth-Scopes` header
- Derives roles from org/team membership
...
The GitHub identity provides additional methods beyond `rocco.Identity`:

```go
id := req.Identity.(*github.Identity)
id.Login()         // GitHub username
id.Name()          // Display name
id.AvatarURL()     // Profile image
id.Organizations() // All orgs
id.Teams()         // All teams (org/team format)
id.Raw()           // Raw GitHub API response
```

Roles are automatically derived:
- `github:user` - all authenticated users
- `org:<name>` - for each organization membership
- `team:<org>/<slug>` - for each team membership

See the [Authentication Cookbook](../4.cookbook/2.authentication.md) for complete GitHub examples.

### OAuth Flow Handlers

The `rocco/github` package validates tokens on incoming requests. To implement the OAuth flow itself (login redirect, callback handling), use the `rocco/oauth` package:

```go
import "github.com/zoobzio/rocco/oauth"

cfg := oauth.GitHub()
cfg.ClientID = os.Getenv("GITHUB_CLIENT_ID")
cfg.ClientSecret = os.Getenv("GITHUB_CLIENT_SECRET")
cfg.RedirectURI = "https://myapp.com/auth/github/callback"
cfg.Scopes = []string{"read:user", "read:org"}

// Application-provided state management (for CSRF protection)
cfg.GenerateState = func(ctx context.Context) (string, error) {
    state := generateSecureRandom(32)
    sessionStore.Set(ctx, "oauth_state", state, 10*time.Minute)
    return state, nil
}
cfg.VerifyState = func(ctx context.Context, state string) (bool, error) {
    expected, err := sessionStore.Get(ctx, "oauth_state")
    if err != nil {
        return false, nil
    }
...
cfg.OnSuccess = func(ctx context.Context, tokens *oauth.TokenResponse) error {
    ...
    return db.StoreUserTokens(ctx, tokens.AccessToken, tokens.RefreshToken)
}
...
The `oauth` package is provider-agnostic. Use `oauth.GitHub()` for GitHub, or configure custom endpoints for other OAuth 2.0 providers.
...
See the [Authentication Cookbook](../4.cookbook/2.authentication.md) for complete OAuth flow examples.
