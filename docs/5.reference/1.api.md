...
Handler names are auto-generated from method and path with a random suffix for uniqueness (e.g., `GET /users/{id}` → `get-users-id-a3f1`).
...
| Parameter | Type | Description |
|-----------|------|-------------|
| `name` | `string` | Handler name for logging and OpenAPI operationId |
...
Sets the validator for request/response validation. Default: `NoOpValidator` (no validation).
...
- `WithValidator(v Validator[In, Out])` - Sets input validator
...
| `Unmarshal(data, v)` | `error` | Decodes bytes into value |
...
Interface for request/response validation. Validators are generic over the handler's input and output types.

| Method | Return | Description |
|--------|--------|-------------|
| `ValidateInput(in)` | `error` | Validates request input before processing |
| `ValidateOutput(out)` | `error` | Validates response output before sending |
...
Default validator that accepts all inputs and outputs (returns `nil` for both methods).
...
Creates a validation error with field-level details. Use this when implementing custom validators to return structured validation errors that rocco can format correctly.
