...
Validation is injectable per-handler via `WithValidator()`. Use any validation library (like `go-playground/validator`) or implement your own. The `validate` struct tag is parsed for OpenAPI schema generation.
