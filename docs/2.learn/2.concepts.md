...
Validation is injectable via the `Validator[In, Out]` interface. By default, no runtime validation occurs (uses `NoOpValidator`).
...
Wrap the popular `go-playground/validator` package:

```go
import "github.com/go-playground/validator/v10"

type GoValidator[In, Out any] struct {
    v *validator.Validate
}

func (g *GoValidator[In, Out]) ValidateInput(in In) error {
    return g.validate(in)
}

func (g *GoValidator[In, Out]) ValidateOutput(out Out) error {
    return g.validate(out)
}

func (g *GoValidator[In, Out]) validate(v any) error {
    if err := g.v.Struct(v); err != nil {
        var ve validator.ValidationErrors
        if errors.As(err, &ve) {
            fields := make([]rocco.ValidationFieldError, len(ve))
            for i, fe := range ve {
                fields[i] = rocco.ValidationFieldError{
                    Field: fe.Field(),
                    Tag:   fe.Tag(),
                    Value: fmt.Sprintf("%v", fe.Value()),
                }
            }
            return rocco.NewValidationError(fields)
        }
        return err
    }
    return nil
...
// Usage
handler.WithValidator(&GoValidator[CreateUserInput, User]{v: validator.New()})
```
